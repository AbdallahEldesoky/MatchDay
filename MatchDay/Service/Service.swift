...
class Service {
    ...
    let headers = ["X-Auth-Token": Default.apiToken]
    let statusCodeRange: Range = 200..<300
    ...
    func getURlForHighlight(completion: @escaping (HighlightDatasource?, Error?) -> Void) {
        
        let highlightHeader = ["X-RapidAPI-Key": "9d1c1a6f12msh2b46b39b08b9a9dp1689f5jsn00b9a0d692f3"]
        let url = URls.highlightBaseURl
        //        let highlightTron = TRON(baseURL: URls.highlightBaseURl)
        //        let request: APIRequest<HighlightDatasource,JsonError> = highlightTron.swiftyJSON.request(".p.rapidapi.com/")
        //        request.headers = highlightHeader
        //        request.method = .get
        //
        //        request.perform(withSuccess: { (highlightDatasource) in
        //
        //            print("successful  request!")
        //            completion(highlightDatasource,nil)
        //
        //        }) { (error) in
        //
        //            print("failure request!")
        ...
        Alamofire.request(URls.highlightBaseURl, method: .get, parameters: nil, encoding: JSONEncoding.default, headers: highlightHeader)
            ...
            .responseJSON { response in
                ...
                case .success(let value):
                    ...
                    let highlightDatasource = try? HighlightDatasource(json: responseJson)
                    completion(highlightDatasource, nil)
                }
        }
